...
const book =  [
  ...
  // 도서 정보 리스트 : Contents props로 전달
const book_list = book.map((book_sub) => (
  {id:book_sub.id, title:book_sub.title, author:book_sub.author, year:book_sub.year, categories:book_sub.categories}
));
  ...
  return (
    <div className="wrap">
      ...
      <Routes>
        ...
        <Route path="/Contents" element={<Contents book_list={book_list}/>}></Route>
        <Route path="/Book/:bookId" element={<Book onFind={
          (id) => {
            // book : 도서 전체 정보 배열
